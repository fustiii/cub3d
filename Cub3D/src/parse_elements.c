...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/27 11:15:17 by gfuster           #+#    #+#             */
/*   Updated: 2026/05/27 11:15:22 by gfuster          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	assign_path(char **target_path, char *path,
	t_game *data, char *err_msg)
{
	if (*target_path != NULL)
	{
...
int	extract_color(t_game *data, char *line)
{
	...
	if (!rgb || validate_color(data, rgb, tokens[1]) == ERROR)
		return (free_and_return(tokens, rgb, ERROR));
	if (ft_strcmp(tokens[0], "F") == 0)
		data->floor_rgb = (atoi(rgb[0]) << 24 | atoi(rgb[1]) << 16
				| atoi(rgb[2]) << 8 | 0xFF);
	else if (ft_strcmp(tokens[0], "C") == 0)
		data->ceil_rgb = (atoi(rgb[0]) << 24 | atoi(rgb[1]) << 16
				| atoi(rgb[2]) << 8 | 0xFF);
	return (free_and_return(tokens, rgb, SUCCESS));
}
